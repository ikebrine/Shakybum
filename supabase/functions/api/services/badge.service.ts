...
import { computeBadge, BUM_OK_BADGES_FOR_ELIGIBILITY } from "../lib/badges.ts";
...
function badgeRank(badge: string) {
  return RANK.indexOf(badge);
}

/**
 * Recomputes and persists a user's badge from their real activity. Call
 * this after anything that changes moves-posted or follower count.
 *
 ...
 */
export async function recomputeBadge(userId: string) {
