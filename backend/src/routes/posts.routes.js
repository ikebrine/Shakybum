...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const post = await postsRepo.create({ userId: req.user.id, videoUrl, caption, moveTag, kind });
  const updatedUser = await recomputeBadge(req.user.id); // a new "move" posted can push moves-count past a badge threshold
...
router.get("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  res.json({ posts: await postsRepo.feed({ kind, limit, before: req.query.before }) });
}));

router.get("/user/:userId", requireAuth, asyncHandler(async (req, res) => {
  const kind = req.query.kind;
  res.json({ posts: await postsRepo.byUser(req.params.userId, { kind }) });
}));

router.get("/:id", requireAuth, asyncHandler(async (req, res) => {
  const post = await postsRepo.findById(req.params.id);
  if (!post) return res.status(404).json({ error: "Not found" });
  res.json({ post, likedByMe: await likesRepo.hasLiked(post.id, req.user.id) });
}));

router.delete("/:id", requireAuth, asyncHandler(async (req, res) => {
  const deleted = await postsRepo.delete(req.params.id, req.user.id);
  if (!deleted) return res.status(404).json({ error: "Not found, or not yours" });
  await recomputeBadge(req.user.id); // deleting a move can drop moves-count back below a threshold
  res.json({ ok: true });
}));
...
router.post("/:id/like", requireAuth, asyncHandler(async (req, res) => {
  const post = await postsRepo.findById(req.params.id);
  if (!post) return res.status(404).json({ error: "Not found" });
  await likesRepo.add(post.id, req.user.id); // idempotent — liking twice is a no-op, not an error
  res.json({ post: await postsRepo.findById(post.id) });
}));

router.delete("/:id/like", requireAuth, asyncHandler(async (req, res) => {
  const post = await postsRepo.findById(req.params.id);
  if (!post) return res.status(404).json({ error: "Not found" });
  await likesRepo.remove(post.id, req.user.id);
  res.json({ post: await postsRepo.findById(post.id) });
}));
...
router.get("/:id/comments", requireAuth, asyncHandler(async (req, res) => {
  const post = await postsRepo.findById(req.params.id);
  if (!post) return res.status(404).json({ error: "Not found" });
  res.json({ comments: await commentsRepo.byPost(post.id) });
}));
...
router.post("/:id/comments", requireAuth, asyncHandler(async (req, res) => {
  ...
  const comment = await commentsRepo.create({ postId: post.id, userId: req.user.id, text: text.trim() });
