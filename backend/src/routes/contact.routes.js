...
router.get("/sent", requireAuth, asyncHandler(async (req, res) => {
  res.json({ requests: await contactRequestsRepo.sentBy(req.user.id) });
}));

router.get("/received", requireAuth, asyncHandler(async (req, res) => {
  res.json({ requests: await contactRequestsRepo.receivedFor(req.user.id, "paid_hold") });
}));

// Reveals the creator's real contact info — only if this specific request is approved.
router.get("/:id", requireAuth, asyncHandler(async (req, res) => {
  const cr = await contactRequestsRepo.findById(req.params.id);
